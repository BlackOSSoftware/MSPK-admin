    ...
    return (
        <div className="dashboard-surface soft-shadow soft-shadow-hover h-full bg-card/90 border border-border/70 rounded-2xl flex flex-col overflow-hidden transition-all duration-300">
            <div className="h-11 shrink-0 border-b border-border/70 px-4 flex items-center justify-between bg-secondary/30">
                <h3 className="text-[11px] font-semibold text-foreground uppercase tracking-[0.2em]">Live Activity</h3>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                {logs.length > 0 ? logs.map((log) => (
                    <div key={log.id} className="group flex items-start gap-3 p-2.5 rounded-lg hover:bg-primary/5 border border-transparent hover:border-border/70 transition-all cursor-default">
                        <div className={`h-7 w-7 rounded-full flex items-center justify-center text-[10px] font-semibold ${getAvatarStyle(log.type)}`}>
                            {getInitials(log)}
                        </div>
                        <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[9px] text-muted-foreground/50 font-medium flex items-center gap-1">
                                <Clock size={9} /> {formatDistanceToNow(new Date(log.time || Date.now()), { addSuffix: true })}
                            </span>
                        </div>
                    </div>
                )) : (
