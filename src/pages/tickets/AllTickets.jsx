...
const AllTickets = () => {
  ...
  const getFilteredTickets = () => {
    let data = tickets;
    if (activeTab === "open")
      data = data.filter((t) => t.status === "Open" || t.status === "OPEN");
      ...
      <div className="h-9 bg-muted/30 border border-border rounded-lg flex items-center justify-between px-4 text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-1">
        <div className="flex items-center gap-4">
          <span>
            ...
            Total:{" "}
            <span className="text-foreground font-bold">{tickets.length}</span>
          </span>
          <div className="ml-4 flex items-center gap-2">
            ...
            >
              ...
              <option value={20} className="bg-card text-foreground">
                20
              </option>
